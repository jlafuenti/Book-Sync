...
def docs_urls(app_env: str) -> dict:
    """Where (or whether) the interactive schema is served (issues #263, #260).

    In prod there is nothing to gain from publishing the full route inventory,
    request shapes and error models to anyone who can reach the port — and the
    published container port is normally reachable on the LAN, so "the reverse
    proxy only forwards /api/*" is not the protection it looks like. In dev the
    Swagger UI is the fastest way to read the API, so it stays on.

    The committed `docs/openapi.json` (regenerated by
    `server/scripts/export_openapi.py`, drift-gated in CI) is what everyone else
    reads; `docs/api.md` is the prose companion.
    """
    if app_env == "prod":
...
async def root():
    ...
    # No "docs" link: the interactive schema is dev-only (see docs_urls above),
    # so advertising it would be a dead link in prod and a signpost everywhere
    # else. The published reference is docs/api.md + docs/openapi.json.
    return {
        "name": "Tandem",
        "version": APP_VERSION,
        "status": "running",
