...
to `en.wiktionary.org` on demand, unauthenticated, on its own OkHttp client with no Bearer
interceptor (`AppModule.provideWiktionaryOkHttpClient`), falling back to `api.dictionaryapi.dev`
(`AppModule.provideDictionaryOkHttpClient`) when Wiktionary has no English entry for the word
(issue #608). No analytics, ads or crash-reporting SDK is present in the build.
...
`server/tests/test_android_no_personal_hosts.py` pins that `en.wiktionary.org` and
`api.dictionaryapi.dev` stay the only hard-coded third-party hosts under `app/src/main/java`.
**A new destination fails that test on purpose** — adding one means updating the policy and the
Data safety answers in [play-listing.md](play-listing.md) in the same change.
