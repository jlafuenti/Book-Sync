...
| **Docker base images** | Runtime *lines* are pinned and move by hand, in one PR, with the CI that tests them: the server's Python line is `server/.python-version` = `tests.yml` = `FROM python:<line>-slim` (contract test); the web image's Node line = `web-tests.yml`'s `node-version`, and it stays on the **active LTS** line, not the newest Current. Dependabot's docker blocks ignore those (Python major/minor, Node major) so the bot only proposes patch tags for them; everything else in a Dockerfile (nginx, ffmpeg-bearing layers) arrives as normal PRs — check the image builds on the deployment host. The Jetson base (`dustynv/faster-whisper`) is tied to the JetPack release on the device; do not take a bump that changes the `r36`/CUDA part without checking the device first. Postgres majors are never proposed; a major there is a planned migration with a backup. |
