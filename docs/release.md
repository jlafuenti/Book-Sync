...
| Internal testing uploads | 0.1.0 (versionCode 100) on 2026-09-14; 0.2.0 (versionCode 200) prepared the same day; 0.2.1 (versionCode 201) on 2026-09-15, the first upload through `./gradlew publishReleaseBundle` rather than the Console; 0.2.2 (versionCode 202) on 2026-09-15; 0.2.3 (versionCode 203) on 2026-09-16, the Android Auto and cache fixes the walk-through is waiting on; 0.3.0 (versionCode 300) on 2026-09-17, the first build carrying the demo sign-in button (#147) as well as the guided walkthrough, the reader selection fixes and the sync-map work. Internal testing is the owner's own test channel and does not count toward the closed-test clock; 0.4.0 (versionCode 400) on 2026-09-18 via `publishReleaseBundle` — the guided walkthrough; 0.4.1 (versionCode 401) on 2026-09-18, repairing Define, which 0.4.0 shipped broken (#631); 0.4.2 (versionCode 402) on 2026-09-18, the realign regression fix (#635) and the Android Auto log attachment (#636); 0.5.0 (versionCode 500) on 2026-09-21 via `publishReleaseBundle` — Android Auto and the player resuming at the reading position (#643), sync-map prefetch with per-device settings (#655), one-request position sync at sign-in (#653), and the first build on Room 23; 0.5.1 (versionCode 501) on 2026-09-21 via `publishReleaseBundle` — the ebook following listening, including across Android Auto, offline and a reader left open (#682), the playback notification opening the player (#684), sync maps kept only for downloaded books (#678), and Readium 3.4; 0.6.0 (versionCode 600) on 2026-09-23 via `publishReleaseBundle` — the streamed player keeping the saved position while a book loads (#706); 0.7.0 (versionCode 700) on 2026-09-26 via `publishReleaseBundle` — Next up on Home (#716), the community Discord link and Report a problem via Discord (#714, #715), and the series link from a book's details (#717) |
| GitHub Release assets | None, by decision (2026-09-21): the app ships through Play only, so GitHub Releases carry notes and no APK or AAB. 0.4.1 and 0.4.2 had already gone out without assets while `releasing.md` still said to attach them; the doc now matches. The last Release with assets is 0.4.0 |
| Closed test first upload | 0.6.0 (versionCode 600) on 2026-09-23, promoted from internal to the alpha (closed testing) track with `./gradlew promoteReleaseArtifact --from-track internal --promote-track alpha`; it replaced 0.1.0 (versionCode 100) on that track. Built with the demo settings, so testers get **Try the demo**. Tester list, invite link and closed-test description are Console-only. 0.7.0 (versionCode 700) on 2026-09-26, promoted the same way; it replaced 0.6.0 |
