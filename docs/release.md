...
| Closed test first upload | 0.6.0 (versionCode 600) on 2026-09-23, promoted from internal to the alpha (closed testing) track with `./gradlew promoteReleaseArtifact --from-track internal --promote-track alpha`; it replaced 0.1.0 (versionCode 100) on that track. Built with the demo settings, so testers get **Try the demo**. Tester list, invite link and closed-test description are Console-only |
