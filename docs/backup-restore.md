...
| **App data** | `./data` bind mount — `covers/` (regenerable but slow), `logs/`, `imports/` (throwaway) | nightly `tar` of `covers/` only |
...
3. Archives covers to `booksync-covers-YYYY-MM-DD.tgz` (skips logs and imports).
4. Prunes old files: keeps the newest `BACKUP_KEEP_DAILY` daily dumps plus 1st-of-month
   dumps up to `BACKUP_KEEP_MONTHLY`.
...
The covers archive is larger but still modest. `14 daily + 6 monthly` is ~20 files.
...
3. The server terminates other DB sessions, runs `pg_restore --clean --if-exists` over
   the live database, and unpacks that date's covers archive if present.
...
# 2. Covers
tar xzf /path/to/nas/booksync-backups/booksync-covers-<DATE>.tgz -C ./data
